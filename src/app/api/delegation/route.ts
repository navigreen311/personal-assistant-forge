...
export async function GET(request: NextRequest) {
  try {
    ...
    const tasks = await getDelegatedTasks(userId, direction);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { taskId, delegatedBy, delegatedTo, contextPack } = parsed.data;
    const pack = contextPack || await buildContextPack(taskId);
    const delegation = await delegateTask(taskId, delegatedBy, delegatedTo, pack);
